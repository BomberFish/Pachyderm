...
extension UIApplication {
    ...
    func confirmAlert(title: String = errorString, body: String, confirmTitle: String = okString, cancelTitle: String = cancelString, onOK: @escaping () -> (), noCancel: Bool = false) {
        DispatchQueue.main.async {
            var body = body
            if title == errorString {
                if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String,
    ...
    func confirmAlert(title: String = errorString, body: String, confirmTitle: String = okString, cancelTitle: String = cancelString, onOK: @escaping () -> (), noCancel: Bool = false) async {
        await MainActor.run {
    ...
    func present(alert: UIAlertController) {
        if var topController = UIApplication.shared.firstWindow?.rootViewController {
...
extension NSApplication {
    ...
    func alertError(_ error: Error, animated: Bool = true) {
        DispatchQueue.main.async {
            #if DEBUG
            self.alert(body: "\(error)", animated: animated)
            #else
            self.alert(body: error.localizedDescription, animated: animated)
            #endif
        }
    }
    ...
    func progressAlert(title: String, body: String = "", animated: Bool = true, noCancel: Bool = true) {
        DispatchQueue.main.async {
            ...
            // NSAlerts presented with runModal() are blocking.
            // A true "progress" alert that allows background tasks and can be dismissed programmatically
            // would require a different presentation (e.g., as a sheet or modeless window)
            // and a way to close it, which is not directly supported by a simple runModal() call.
            // For now, this will show an alert with a spinner, but it will still block the main thread.
            // If a cancel button is needed, it can be added, but dismissing it programmatically while runModal is active is not straightforward.
            if !noCancel {
                alert.addButton(withTitle: cancelString)
            }
            
            // Due to runModal(), this alert will be blocking.
            // Consider using beginSheetModal for non-blocking alerts if UI interaction is needed during progress.
            alert.runModal()
        }
    }
